...
const Home: FC = () => {
  ...
  const handleConnect = async () => {
    ...
    const sessionResponse = await fetch("/api/talks/stream", {method: 'POST'})
  ...
  const handleTalk = async () => {
    ...
    if (peerConnectionRef.current?.signalingState === 'stable' || peerConnectionRef.current?.iceConnectionState === 'connected') {
      const talkResponse = await fetch(`/api/talks/stream/${streamIdRef.current}`, {
        method: 'POST',
        body: JSON.stringify({
          input: input,
  ...
  const createPeerConnection = async (offer: RTCSessionDescriptionInit, iceServers: []) => {
    ...
    console.log('[createPeerConnection] peerConnectionRef.current=', peerConnectionRef.current);
    ...
    <main className="flex min-h-screen flex-col items-center justify-between p-24">
      <div className="content">
        <div className="video-wrapper">
          <div>
            <video src="/videos/or_idle.mp4" ref={talkVideoRef} id="talk-video" width="400" height="400"
                   autoPlay></video>
          </div>
        </div>
        <div id="status">
          ICE gathering status: <label id="ice-gathering-status-label"
                                       className={`iceGatheringState-${iceGatheringStatusLabel}`}>{iceGatheringStatusLabel}</label
        ><br/>
          ICE status: <label id="ice-status-label"
                             className={`iceConnectionState-${iceStatusLabel}`}>{iceStatusLabel}</label><br/>
          Peer connection status: <label id="peer-status-label"
                                         className={`peerConnectionState-${peerStatusLabel}`}>{peerStatusLabel}</label><br/>
          Signaling status: <label id="signaling-status-label"
                                   className={`signalingState-${signalingStatusLabel}`}>{signalingStatusLabel}</label><br/>
          Streaming status: <label id="streaming-status-label"
                                   className={`streamingState-${streamingStatusLabel}`}>{streamingStatusLabel}</label><br/>
        </div>
        <br/>
        <div className="buttons">
          <button id="connect-button" type="button" onClick={() => {
            handleConnect()
          }}>Connect
          </button>

          <button id="destroy-button" type="button" onClick={() => {
            handleDestroy()
          }}>Destroy
          </button>
        </div>
        ...
        <div className="flex items-center mt-8 m-auto">
          <input className="p-3" value={input} onChange={(e) => {setInput(e.target.value)}}/>

          <button id="talk-button" type="button" onClick={() => {
            handleTalk()
          }}>Start
          </button>
        </div>
